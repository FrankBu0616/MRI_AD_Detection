...
class AD_Standard_2D3AxisImage(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        AXimageChann = axKeyChannel(image)
        CORimageChann = corKeyChannel(image)
        SAGimageChann = sagKeyChannel(image)
...
def getKeyChannel(image_array, keyIndex, section):
    slice_i = keyIndex
    slice_select = eval("image_array" + section)
    return slice_select

def axKeyChannel(image):
    image_array = np.array(image.get_data())
    return getKeyChannel(image_array, AX_INDEX, AX_SCETION)


def corKeyChannel(image):
    image_array = np.array(image.get_data())
    return getKeyChannel(image_array, COR_INDEX, COR_SCETION)


def sagKeyChannel(image):
    image_array = np.array(image.get_data())
    return getKeyChannel(image_array, SAG_INDEX, SAG_SCETION)
